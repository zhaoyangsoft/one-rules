...
export class Rule extends Lint.Rules.AbstractRule {
  ...
  public static metadata: Lint.IRuleMetadata = {
    ...
    description: Lint.Utils
      .dedent`Warn if an element that likely requires a key prop — namely, \
...
function walk(ctx: Lint.WalkContext<void>): void {
  return ts.forEachChild(ctx.sourceFile, function cb(node: ts.Node): void {
    ...
    if (isPropertyAccessExpression(node) && node.name.text === "map") {
      const mapFn =
        node.parent !== undefined && isCallExpression(node.parent)
          ? node.parent.arguments[0]
          : undefined;

      if (
        mapFn !== undefined &&
        (isArrowFunction(mapFn) || isFunctionExpression(mapFn))
      ) {
...
function getReturnStatement(body: ts.NodeArray<ts.Statement>) {
  return body.filter(item => isReturnStatement(item))[0] as
    | ts.ReturnStatement
    | undefined;
}
