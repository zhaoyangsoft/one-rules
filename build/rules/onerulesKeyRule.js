...
var Rule = /** @class */ (function (_super) {
    ...
    Rule.metadata = {
        ruleName: "onerules-key",
        description: Lint.Utils
            .dedent(templateObject_1 || (templateObject_1 = __makeTemplateObject(["Warn if an element that likely requires a key prop \u2014 namely,             one present in an array literal or an arrow function expression."], ["Warn if an element that likely requires a key prop \u2014 namely, \\\n            one present in an array literal or an arrow function expression."]))),
        options: null,
        optionsDescription: "",
        optionExamples: ["true"],
        type: "functionality",
    ...
    return ts.forEachChild(ctx.sourceFile, function cb(node) {
        ...
        if (tsutils_1.isPropertyAccessExpression(node) && node.name.text === "map") {
            var mapFn = node.parent !== undefined && tsutils_1.isCallExpression(node.parent)
                ...
                : undefined;
            if (mapFn !== undefined &&
                (tsutils_1.isArrowFunction(mapFn) || tsutils_1.isFunctionExpression(mapFn))) {
...
function hasKeyPropSpread(attributes) {
    return attributes.properties.some(function (prop) {
        return tsutils_1.isJsxSpreadAttribute(prop) &&
            ...
            prop.expression.properties.some(function (expProp) {
                return expProp.name !== undefined &&
                    tsutils_1.isIdentifier(expProp.name) &&
                    expProp.name.text === "key";
            });
    });
}
